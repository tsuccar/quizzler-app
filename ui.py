...
class QuizInterface:
	...
	def __init__(self,quiz_brain:QuizBrain):
		...
		self.true_button = Button(image=true_button_image, bg=THEME_COLOR, highlightthickness=0,command=self.true_check)
		self.true_button.grid(column=0, row=2)
		
		false_button_image = PhotoImage(file="./images/false.png")
		self.false_button = Button(image=false_button_image, highlightthickness=0,command=self.false_check)
	...
	def get_next_question(self):
		...
		if self.quiz.still_has_questions():
			...
			#to disable the buttons once the head has reached.
